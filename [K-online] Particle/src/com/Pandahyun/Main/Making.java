...
public class Making {
	...
	public static void makeField(String name, Player _p, Location lc, World world)
	{
		...
		{
			_p.sendMessage(ChatColor.AQUA + "[KPT]" + ChatColor.RED + " �̹� �ִ� �ʵ��Դϴ�.");
			return;
		}*/
			...
			for (int j = 0; j < 3 * 16 * ratio; ++j)
			{
				for (int k = -16 * ratio; k < 16 * ratio; ++k)
				{
					block = world.getBlockAt(
							 ...
							 (int)lc.getY() + j-1,
							 (int)lc.getZ() + k);
					if(i==-1*16*ratio || i == 16*ratio-1||j == 0 || j == 3*16*ratio-1 || k==-16*ratio || k==16*ratio-1)
		...
		if (_p!=null)
			_p.sendMessage(ChatColor.AQUA + "[KPT] " + ChatColor.GREEN + name + " �ʵ� ����");
	...
	public static void removeField(String name, Player _p, World world, Boolean b)
	{
		if(!main.centers.containsKey(name))
		{
			_p.sendMessage(ChatColor.AQUA + "[KPT]" + ChatColor.RED + " ���� �ʵ��Դϴ�.");
			return;
		}
	...
	public static void scanParticle(String name, File file, String saveName, Player _p, World world)
	{
		...
		FileWriter fw;
		try {
			fw = new FileWriter(new File(file, saveName + ".pts"));
			...
			for (int i = -16 * ratio; i < 16 * ratio; ++i)
			{
				for (int j = 0; j < 3 * 16 * ratio; ++j)
				{
					for (int k = -16 * ratio; k < 16 * ratio; ++k)
					{
						...
						{
							fb.append("" + i / (16D * ratio * -1));
							
							fb.append(" " + j / (16D * ratio));
							
							fb.append(" " + k / (16D * ratio));
							fb.newLine();
						}
					}
				}
